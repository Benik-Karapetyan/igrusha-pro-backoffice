...
import { ScannerFormSchema, ScannerFormValues } from "./ScannerForm.consts";

interface ScannerFormProps {
  onSuccess: () => void;
}

export const ScannerForm: FC<ScannerFormProps> = ({ onSuccess }) => {
  const toast = useToast();
  const firstInputRef = useRef<HTMLInputElement>(null);
  const dialogMode = useStore((s) => s.dialogMode);
  const setDialogs = useStore((s) => s.setDialogs);
  const defaultValues = useStore((s) => s.scanner);
  const form = useForm({
    defaultValues,
    validators: {
      onSubmit: ScannerFormSchema,
    },
    onSubmit: ({ value }) => {
      if (dialogMode === "create") createScanner(value);
      else updateScanner(value);
    },
  });
  ...
  const handleClose = () => {
    if (hasUnsavedChanges) setDialogs(["scanner", "unsavedChanges"]);
    else setDialogs([]);
  };
  ...
  const createScanner = async (requestData: ScannerFormValues) => {
    try {
      setLoading(true);
      await api.post("/bo/api/scanners", requestData);
      setDialogs([]);
  ...
  const updateScanner = async (requestData: ScannerFormValues) => {
    try {
      setLoading(true);
      await api.put(`/bo/api/scanners/${requestData.id}`, requestData);
      setDialogs([]);
        ...
        <Subscribe selector={({ canSubmit }) => [canSubmit]}>
          {([canSubmit]) => (
            <Button type="submit" className="w-[160px]" disabled={!canSubmit} loading={loading}>
              {dialogMode === "create" ? "Add Scanner" : "Update Scanner"}
            </Button>
          )}
        </Subscribe>
      </DialogFooter>
