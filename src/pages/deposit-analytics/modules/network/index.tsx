...
import { DepositNetworkFiltersForm, DepositNetworkFiltersFormValues, emptyDepositNetworkFilters } from "@forms";
...
export const Network = () => {
  ...
  const handleFilter = useCallback((filters: DepositNetworkFiltersFormValues) => {
    setParams((prev) => ({ ...prev, ...filters }));
    canFetch.current = true;
  }, []);

  const getDepositsByNetwork = useCallback(async () => {
    try {
      setLoading(true);
      ...
      const { data } = await api.get(`/analytics/api/inflowOutflow/deposit`, {
        params: searchParams,
      });
  ...
  useEffect(() => {
    if (canFetch.current) {
      ...
      void getDepositsByNetwork();
    }
  }, [getDepositsByNetwork]);
  ...
  return (
    <div>
      <div className="flex items-center justify-between p-4 pb-0">
        ...
        <DepositNetworkFiltersForm onChange={handleFilter} />
      </div>
