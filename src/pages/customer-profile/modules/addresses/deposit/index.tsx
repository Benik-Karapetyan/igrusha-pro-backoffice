...
import { useDepositHeaders } from "./hooks/useDepositHeaders";

export const Deposit = () => {
  const customerMainInfo = useStore((s) => s.customerMainInfo);
  const { headers } = useDepositHeaders();
  ...
  const getDeposits = useCallback(async () => {
    try {
      ...
      const { data } = await api.get(`/analytics/api/inflowOutflow/deposit/${customerMainInfo?.identityId}`, {
        params,
      });
