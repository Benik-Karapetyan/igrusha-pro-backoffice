...
export const useBrandHeaders = () => {
  ...
  const actions = useMemo(() => {
    ...
    if (checkPermission("brand_update")) tableActions.push("edit");
    if (checkPermission("brand_delete")) tableActions.push("delete");

    return tableActions;
  }, [checkPermission]);

  const handleEdit = (item: TableItem) => {
    setBrand(item as BrandFormValues);
  ...
  if (checkPermission("brand_update")) {
    headers.splice(2, 0, {
      text: "enabled",
      value: (item) =>
