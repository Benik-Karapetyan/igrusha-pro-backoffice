...
import { CoinFormValues } from "@forms";
import { useCheckPermission } from "@hooks";
import { useStore } from "@store";
import { HeaderItem, TableItem } from "@ui-kit";

export const useCoinHeaders = () => {
  ...
  const actions = useMemo(() => {
    ...
    if (checkPermission("coin_update")) tableActions.push("edit");
    if (checkPermission("coin_delete")) tableActions.push("delete");

    return tableActions;
  }, [checkPermission]);

  const handleEdit = (item: TableItem) => {
    setCoin(item as CoinFormValues);
  ...
  const headers: HeaderItem[] = [
    ...
    { text: "derive path", value: "derivePath" },
    {
      text: "node",
      value: (item) => (typeof item?.node === "object" && typeof item?.node?.name === "string" ? item?.node?.name : ""),
    },
    {
      text: "status",
  ...
  if (checkPermission("coin_update")) {
    headers.splice(5, 0, {
      text: "enabled",
      value: (item) =>
